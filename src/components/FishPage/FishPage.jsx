...
const FishPage = () => {
    ...
    useEffect(() => {
        if (isFishAvailableToPurchase) {
            setIsBuyBtnDisabled(false);
        } else {
            setIsBuyBtnDisabled(true);
        }
    }, [isFishAvailableToPurchase]);
    ...
    const startPaymentProcess = async () => {
        if (!window.ethereum && !window?.web3?.currentProvider) {
            ...
            enqueueSnackbar(
                <div>
                    You need to connect{' '}
                    <Link underline="always" href={metamaskUrl} style={{ color: '#fff' }}>
    ...
    const getBtnTitle = () => {
        let title = 'SELECT & BUY';

        if (!isFishAvailableToPurchase) {
            title = 'NOT FOR SALE';
        }

        if (!isFishAvailableToPurchase && isCurrentUserOwner) {
            title = 'SOLD';
        }
